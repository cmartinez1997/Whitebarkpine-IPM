...
# okay now detrend series with a cubic smoothing spline and then make a chronology 
...
climate_dat_WY <- read_csv("data_processed/climate_data_all.csv")
head(climate_dat_WY)
climate_dat_WY$PLT_CN <- as.character(climate_dat_WY$PLT_CN)

# tree ring 
dat_WY <- tree_ring_usable %>%
  filter(STATECD == 56) %>% 
  dplyr::select(TRE_CN, PLT_CN, Year, RW)

# filter 
climate_dat_WY <- climate_dat_WY[climate_dat_WY$PLT_CN %in% dat_WY$PLT_CN, ] 
climate_dat_WY$PLT_CN <- as.character(climate_dat_WY$PLT_CN)
climate_dat_WY <- climate_dat_WY %>% 
  filter(year > 1895)


tmax_WY <- climate_dat_WY %>%
  dplyr::select(year, contains("tmax")) %>%
  pivot_longer(-year,names_to = "var1",values_to = "tmax")

tmean_WY <- climate_dat_WY %>%
...
window_range <- c(12, 0)  # Range of windows to be included in the analysis
