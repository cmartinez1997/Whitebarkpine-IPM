...
model_growth_data <- model_growth_data %>% dplyr::select(TRE_CN, PREV_TRE_CN, PLT_CN, PREV_PLT_CN, LAT, LON, ELEV, 
                                                  DIA, PREVDIA, DIA_DIFF, MEASYEAR, PREV_MEASYEAR, CENSUS_INTERVAL, 
                                                  BALIVE, CONDID, STATUSCD, DSTRBCD1, DSTRBCD2, DISTRBCD3, AGENTCD)

#scale numerical covariates (just BALIVE at this point, lat, lon, elev)

scaled_grow_data <- grow_data_model %>% mutate_at(scale, .vars = vars(-TRE_CN, -PREV_TRE_CN, -PLT_CN, -PREV_PLT_CN, -CONDID,
