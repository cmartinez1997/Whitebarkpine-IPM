##Making WBP rw data frame with metadata for flathead county 
...
tree_dat_wbp <- tree_dat %>% filter(SPCD == 101) #19,611 WBP trees 
...
write_csv(tree_dat_wbp, "data_processed/tree_dat_wbp_mt.csv")
##merge the inventory data from FIAdb with tree ring 

tree_dat_wbp <- read_csv("data_processed/tree_dat_wbp_mt.csv")

mt_flathead_total <-  wbp_meta %>% 
  filter(STATECD == 30) %>% 
  filter(COUNTYCD == 29) # so 20 samples 
#need to filter trees from specific counties for our analysis - only include flathead county

mt_meta_flathead <- wbp_meta %>% 
  filter(!is.na(TRE_CN) & !is.na(PLT_CN)) %>% 
  filter(STATECD == 30) %>% 
  filter(COUNTYCD == 29) #only 16 samples have a tree cn associated with them

mt_meta_flathead_fiadb <- left_join(mt_meta_flathead, tree_dat_wbp, by = "TRE_CN")
...
wbp_meta$TRE_CN <- as.character(wbp_meta$TRE_CN)
wbp_meta$PLT_CN <- as.character(wbp_meta$PLT_CN)
...
mt_rw_flathead <- wbp_rw %>% 
  filter(!is.na(TRE_CN)) %>% 
  filter(STATECD == 30) %>% 
  filter(COUNTYCD == 29) #79 cores from these counties for our analysis

mt_rw_flathead <- left_join(mt_rw_flathead, mt_meta_flathead_fiadb)

mt_meta_flathead_fiadb$TRE_CN <- as.character(mt_meta_flathead_fiadb$TRE_CN)
...
# reread in the data
mt_rw_flathead <- read_csv("data_processed/mt_rw_flathead.csv")
mt_meta_flathead <- read_csv("data_processed/mt_meta_flathead_fiadb.csv")

mt_rw_flathead_df <- mt_rw_flathead %>% 
  select(CN, TRE_CN, PLT_CN, Year, RW, STATECD, COUNTYCD, PLOT, SUBP, TREE, SPCD, TREE, DIA, MEASYEAR, PITH, RINGCOUNT, CONDID, CR, CCLCD, PREV_PLT_CN, 
         LAT, LON, REMEAS_PERIOD, ELEV, BALIVE)

mt_meta_flathead_df <- mt_meta_flathead %>% 
  select(CN, TRE_CN, PLT_CN, STATECD, COUNTYCD, PLOT, SUBP, TREE, SPCD, TREE, DIA, MEASYEAR, PITH, RINGCOUNT, CONDID, CR, CCLCD, PREV_PLT_CN, 
         LAT, LON, REMEAS_PERIOD, ELEV, BALIVE)



#read in the rwl files that Ceci measured/remeasured for sample 44
mt_44_df <- read_tsv("data_processed/montana_fia_rwl/county29_flathead/044-4-3.rwl")
mt_44 <- read.tucson("data_processed/montana_fia_rwl/county29_flathead/044-4-3.rwl")
head(mt_44)
...
mt_44_df <- data.frame(Year = as.numeric(row.names(mt_44)), RW = mt_44$RW)
str(mt_44_df)

str(mt_44_df)
mt_44_usu <- mt_rw_flathead_df %>% 
  filter(CN == 289859799489998) %>% 
  select(Year, RW) 
